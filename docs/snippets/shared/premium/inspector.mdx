...
- Provide `publicApiKey` (for Copilot Cloud) or `publicLicenseKey` (for self-hosted) to `<CopilotKit />` to enable premium features:

```tsx
<CopilotKit 
  publicApiKey={process.env.NEXT_PUBLIC_COPILOTKIT_API_KEY} // Use publicApiKey for Copilot Cloud
  // OR
  publicLicenseKey={process.env.NEXT_PUBLIC_COPILOTKIT_LICENSE_KEY} // Use publicLicenseKey for self-hosted
>
  {children}
</CopilotKit>
```
