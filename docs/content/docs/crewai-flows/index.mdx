...
<FrameworkOverview
  ...
  supportedFeatures={[
    {
      ...
      // CrewAI Flows has its own Generative UI docs under /crewai-flows (using /coagents would redirect to /langgraph per next.config.mjs).
    ...
    },
    {
      ...
      // Point to CrewAI Flows HITL docs (the /coagents path is reserved for LangGraph and is redirected there).
    ...
    },
    {
      ...
      // Point to CrewAI Flows Shared State docs (avoids redirecting to LangGraph's shared state docs).
  ...
  // CrewAI Flows uses an architecture video (FrameworkOverview supports the `architectureVideo` prop).
