...
For this tutorial, we'll use the `coagents-starter` example.

In a separate terminal, run the following command to start the example UI:
...
<Callout type="info">
We use the `pnpm run dev` command to run the example UI, which is different from the `turbo run dev` command we use to work on the individual packages.
</Callout>

Now navigate to http://localhost:3000 and you should see the example running. Any changes you make to the CopilotKit packages will immediately be reflected here.
