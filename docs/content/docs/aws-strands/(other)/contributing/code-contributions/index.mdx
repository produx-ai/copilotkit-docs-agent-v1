...
```bash
# Ensure you're using pnpm v10 to match the repo's package manager version
# and Node.js 20.x or later, as older versions may fail the build.
turbo run build
```
</Steps>
...
> **Note:** The script path has changed; use the `scripts/qa/lib/langserve` helper instead of `scripts/develop/example.sh`, which no longer exists.

Next, go to the `CopilotKit/` directory and run the helper script for the example you want to work on:

```bash
# Example: run the coagents-starter example locally with hot reload
pnpm --filter @copilotkit/scripts-qa exec node scripts/qa/lib/langserve/index.js coagents-starter
```
...
```bash
pnpm --filter @copilotkit/scripts-qa exec node scripts/qa/lib/langserve/index.js coagents-starter langgraph-platform
```
