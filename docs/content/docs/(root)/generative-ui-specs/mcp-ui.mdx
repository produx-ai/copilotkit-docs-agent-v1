...
*Note: The `{ type: "mcp-ui", spec: ... }` payload shape is not implemented in the current SDK. The AG-UI protocol presently focuses on agent integrations; verify support before using this structure.*
