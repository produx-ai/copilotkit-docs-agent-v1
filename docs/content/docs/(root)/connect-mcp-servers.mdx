  ...
  >
    ...
    >
      ...
      <Step>
        ...
        <Callout type="info" title="MCP server management is coming soon">
          The current `@copilotkit/react-core` release does not export `setMcpServers` yet. MCP server management will be added in an upcoming release. For now, follow runtime-level MCP configuration in your backend or check the release notes for updates.
        </Callout>
      </Step>
      <Step>
