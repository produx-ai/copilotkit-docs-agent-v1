...
{/* Feature viewer links will be added once Microsoft Agent Framework is available there. */}
<Callout type="warning">
  Live demo and code previews will be linked here when the Microsoft Agent Framework examples are added to the CopilotKit feature viewer.
</Callout>
    ...
    <Step>
        ...
        export function Page() {
          ...
          useFrontendTool({
            ...
            render: ({ result, args }) => {
              // Renders UI based on the data of the tool call and shows the handler result
              const { currentURLPath, userName } = result || {};
              return (
                <div>
                  <h1>Hello, {args.name}!</h1>
                  {result ? (
                    <p>Tool called with result: User '{userName}' on page '{currentURLPath}'</p>
                  ) : (
                    <p>Tool not yet called or failed.</p>
                  )}
                </div>
              );
            },
