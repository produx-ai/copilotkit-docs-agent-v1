...
This guide assumes you have Google's Assistant Development Kit (ADK) set up in your environment. If you haven't installed ADK yet, follow these steps:

- Install the required Python packages:
  ```
  pip install ag-ui-adk google-adk  # These are Google-provided libraries, not part of CopilotKit core.
  ```
- Ensure access to Google's ADK services and a valid API key configured.
- If you're not using ADK, refer to other CopilotKit integrations like [LangGraph](/docs/langgraph) or standalone runtime setup.
