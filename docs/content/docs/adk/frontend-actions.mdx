...
This can be utilized to let your agent control the UI, [generative UI](/adk/generative-ui/frontend-tools), or for Human-in-the-loop interactions.
    ...
    <Step>
        ...
        export function Page() {
          ...
          useFrontendTool({
            ...
            handler: async ({ name }) => {
              console.log(`Hello, ${name}!`); // Less intrusive: log for demos or debugging
              // Or update UI state: setMessage(`Hello, ${name}!`);
            },
          });
