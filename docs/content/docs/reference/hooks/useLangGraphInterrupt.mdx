...
<PropertyReference name="action" type="Action" required>
  ...
  The action to perform when an Interrupt event is emitted. Either `handler` or `render` must be defined as arguments
    ...
    <PropertyReference name="handler" type="(props: LangGraphInterruptRenderHandlerProps<TEventValue>) => any | Promise<any>">
      A handler to programmatically resolve the Interrupt, or perform operations whose result will be passed to the `render` method.
    </PropertyReference>
