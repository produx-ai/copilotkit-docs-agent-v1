...
The value to be added to the Copilot context. Object values are automatically stringified using `JSON.stringify`. Note: this does not handle circular references—provide a custom `convert` function for complex objects.
</PropertyReference>
</PropertyReference>
<PropertyReference name="parentId" type="string"  > 
The ID of the parent context, if any. Creates a hierarchical tree structure where child contexts are nested under parents. This hierarchy is formatted as indented, numbered text and provided to the LLM for better context understanding.
</PropertyReference>
</PropertyReference>
<PropertyReference name="categories" type="string[]"  > 
An array of categories to control which context are visible where. Particularly useful with CopilotTextarea—see the [`useMakeAutosuggestionFunction`](/reference/hooks/useMakeAutosuggestionFunction) hook.
</PropertyReference>
</PropertyReference>
