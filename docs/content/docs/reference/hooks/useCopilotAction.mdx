...
<PropertyReference name="action" type="Action" required>
  The function made available to the Copilot. See [Action](#action).
    ...
    <PropertyReference name="available" type="'enabled' | 'disabled'">
      Use this property to control when the action is available to the Copilot. The legacy "remote" value is not supported in replacement hooks and should be migrated to "enabled"/"disabled" semantics.
    </PropertyReference>
    </PropertyReference>
...
**Migration note**: The `"remote"` value for `available` was removed in the newer hooks. Use `"enabled"` or `"disabled"` going forward; any legacy `"remote"` usage should be treated as `"enabled"` when migrating.
          The arguments passed to the action in real time. When the status is `"inProgress"`, they are
          possibly incomplete.
        </PropertyReference>
