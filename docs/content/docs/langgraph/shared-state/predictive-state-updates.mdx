                    ...
                    <Tab value="TypeScript">
                        ...
                        async function chat_node(state: AgentState, config: RunnableConfig) {
                            ...
                            for (const step of steps) {
                                const observed_steps = [...(state.observed_steps ?? []), step];
                                await copilotkitEmitState(config, { ...state, observed_steps }); // docs-fix: `copilotkitEmitState` emits the provided state object; pass an explicit updated snapshot instead of relying on in-place mutation.
                    ...
                    <Tab value="TypeScript">
                        ...
                        async function frontendActionsNode(state: AgentState, config: RunnableConfig): Promise<AgentState> {
                            ...
                            if (response.tool_calls?.length) {
                                return {
                                    messages: response,
                                    // docs-fix: tool call `args` may be returned as a JSON string depending on the model/provider; parse defensively before reading `steps`.
                                    observed_steps: (typeof response.tool_calls[0].args === "string"
                                      ? JSON.parse(response.tool_calls[0].args)
