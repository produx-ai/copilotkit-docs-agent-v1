...
internal state throughout execution, which you can access via the `useCoAgentStateRender` hook (for rendering) or the `useCoAgent` hook (for reading/writing). // docs-fix: `useCoAgentState` is not a current hook; the public hooks are `useCoAgentStateRender` and `useCoAgent`.
