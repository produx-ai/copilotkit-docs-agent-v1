...
import { TextMessage, Role } from "@copilotkit/runtime-client-gql";  // [!code highlight] {/* docs-fix: `Role` is the stable exported alias for message roles from `@copilotkit/runtime-client-gql`. */}
...
See **[predictive state updates](/langgraph/shared-state/predictive-state-updates).**{/* docs-fix: update stale `/coagents/` link to the current `/langgraph/` docs route. */}
