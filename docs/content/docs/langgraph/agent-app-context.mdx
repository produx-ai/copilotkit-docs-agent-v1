        ...
        <Tabs groupId="language_langgraph_agent" items={['Python', 'TypeScript']} default="Python" persist>
            ...
            <Tab value="TypeScript">
                ...
                async function chat_node(state: AgentState, config: RunnableConfig) {
                    // Extract the colleagues from CopilotKit context
                    const copilotKitContext = state.copilotkit.context // docs-fix: CopilotKit state is stored under `copilotkit` (lowercase) in CopilotKitStateAnnotation.
                    const colleaguesContextItem = copilotKitContext.find(contextItem => contextItem.description === "The current user's colleagues") // docs-fix: removed stray quote so the example matches the description used in useCopilotReadable.
                    // Provide the list of colleagues to the LLM
