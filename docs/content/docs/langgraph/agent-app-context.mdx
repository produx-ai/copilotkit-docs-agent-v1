        ...
        <Tabs groupId="language_langgraph_agent" items={['Python', 'TypeScript']} default="Python" persist>
            ...
            <Tab value="TypeScript">
                ...
                async function chat_node(state: AgentState, config: RunnableConfig) {
                    // Extract the colleagues from CopilotKit context
                    const copilotKitContext = state.copilotkit.context; // NOTE: the CopilotKit state key is `copilotkit` (lowercase) when using `CopilotKitStateAnnotation`.
                    const colleaguesContextItem = copilotKitContext.find(
                      (contextItem) => contextItem.description === "The current user's colleagues" // NOTE: keep the description string consistent with the `useCopilotReadable` registration.
                    );
                    const colleagues = colleaguesContextItem?.value ?? []; // NOTE: guard against missing context to avoid runtime errors in this example.
