...
<!-- Docs fix: verified `useHumanInTheLoop` is exported from `@copilotkit/react-core`; adjusted example to reflect `respond`/`status` behavior. -->
...
<Steps>
    ...
    <Step>
        ...
        <!-- Docs fix: grammar ("an approval" instead of "a approval") per verification report. -->
        ...
        export function Page() {
          ...
          useHumanInTheLoop({
            ...
            render: ({ args, respond, status }) => {
              // Docs fix: `respond` is only available when the HITL tool is actively executing.
