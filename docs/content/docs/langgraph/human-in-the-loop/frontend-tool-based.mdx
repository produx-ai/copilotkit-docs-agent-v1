...
<Steps>
        ...
        In this example we will simulate a "approval" flow for executing a command. Use the `useCopilotAction` hook with `renderAndWaitForResponse` to create a tool that
    ...
    <Step>
        ...
        Here's how you can call a frontend tool from your agent:
            <Tab value="TypeScript">
                ```typescript title="agent-js/src/agent.ts"
                import { convertActionsToDynamicStructuredTools } from "@copilotkit/sdk-js"; // [!code highlight]

                async function agentNode(state: YourAgentState, config: RunnableConfig): Promise<YourAgentState> {
                    // Access the tools from the copilotkit property and convert them
