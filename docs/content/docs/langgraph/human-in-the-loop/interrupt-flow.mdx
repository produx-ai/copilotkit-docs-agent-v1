...
description: Learn how to implement Human-in-the-Loop (HITL) using an interrupt-based flow.
---

<!-- Docs fix: corrected frontmatter grammar ("an interrupt-based flow") per verification report. -->
...
interaction (see [Node-based](/langgraph/human-in-the-loop/node-flow) if you need the agent to be aware of HITL context), however [CopilotKit's SDKs provide helpers to alleviate this](/reference/hooks/useLangGraphInterrupt).

<!-- Docs fix: the previous in-page anchor pointed to a non-existent section; link updated to an existing, relevant reference. -->
...
<Step>
    ...
    <Callout type="info">
        ...
        If you want the agent to be aware of HITL context, see the [Node-based](/langgraph/human-in-the-loop/node-flow) guide. For rendering and responding to interrupts in the UI, see [`useLangGraphInterrupt`](/reference/hooks/useLangGraphInterrupt).
        <!-- Docs fix: replaced broken in-page anchor link with links to existing docs that cover the two relevant concepts (agent awareness vs UI handling). -->
    </Callout>
...
<Step>
    ...
    useLangGraphInterrupt({
        render: ({ event, resolve }) => (
            <div>
                {/* Docs fix: `event.value` directly reflects whatever payload was passed to `interrupt(...)` in the agent (string, object, etc.). */}
                <p>{event.value}</p>
                <form onSubmit={(e) => {
                    // Docs fix: keep the default form submit prevented so the page doesn't reload during an interrupt response.
                    e.preventDefault();
...
<Steps>
    <Step>
        ...
        First, let's define two different interrupts. We will include a "type" property to differentiate them, and pass an object payload so the frontend can read the same shape via `event.value`.
        <!-- Docs fix: clarify that the interrupt event value in the UI mirrors whatever payload is passed to `interrupt(...)` in the agent. -->
        <Tabs groupId="language_langgraph_agent" items={['Python', 'TypeScript']} default="Python" persist>
            <Tab value="Python">
                ```python title="agent.py"
                from langgraph.types import interrupt # [!code highlight]
