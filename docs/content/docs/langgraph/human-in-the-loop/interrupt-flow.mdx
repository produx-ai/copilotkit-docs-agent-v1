        ...
        <Tab value="Python">
            ...
            def chat_node(state: AgentState, config: RunnableConfig):
                if not state.get("agent_name"):
                    ...
                    agent_name, new_messages = copilotkit_interrupt(message='Before we start, what would you like to call me?') # [!code highlight]
                    state["agent_name"] = agent_name # [!code highlight]
                    state["messages"] = state["messages"] + new_messages # [!code highlight]
...
When rendering multiple `interrupt` events in the agent, there could be conflicts between multiple `useLangGraphInterrupt` hooks calls in the UI.
            <Tab value="Python">
                ...
                def chat_node(state: AgentState, config: RunnableConfig):
                  ...
                  approval, approval_messages = copilotkit_interrupt(message={ "type": "approval", "content": "please approve" }) # [!code highlight]
                  state["approval"] = approval # [!code highlight]
                  state["messages"] = state["messages"] + approval_messages # [!code highlight]

                  if not state.get("agent_name"):
                    # Interrupt and wait for the user to respond with a name
                    agent_name, name_messages = copilotkit_interrupt(message={ "type": "ask", "content": "Before we start, what would you like to call me?" }) # [!code highlight]
                    state["agent_name"] = agent_name # [!code highlight]
                    state["messages"] = state["messages"] + name_messages # [!code highlight]
