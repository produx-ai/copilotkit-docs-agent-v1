...
interaction, however [CopilotKit's SDKs provide helpers to alleviate this](/langgraph/human-in-the-loop/node-flow). <!-- PATCH: Fix broken in-page anchor link (no matching header); point readers to the existing node-based doc that covers agent-aware HITL. -->
## Implementation
...
<Step>
    ...
    <Callout type="info">
        ...
        If you want this behavior, see the [node-based human-in-the-loop guide](/langgraph/human-in-the-loop/node-flow). <!-- PATCH: Fix broken in-page anchor link (no matching header); route to existing doc describing agent-aware HITL. -->
    <Tabs groupId="language_langgraph_agent" items={['Python', 'TypeScript']} default="Python" persist>
        <Tab value="Python">
            ```python title="agent.py"
...
const YourMainContent = () => {
    ...
    useLangGraphInterrupt({
        handler: async ({ result, event, resolve }) => {
            const { id: userId, department } = await getUserByEmail(userEmail.email) // PATCH: Fix undefined `userId` and pass the correct email string into lookup.
            ...
            return { department, userId } // PATCH: `userId` now comes from getUserByEmail, matching the shape used later in `result.userId`.
        },
