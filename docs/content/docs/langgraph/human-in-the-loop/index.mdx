...
<Callout>
  This video shows an example of our [AI Travel App](/langgraph/tutorials/ai-travel-app) using HITL to get user feedback. {/* docs-fix: tutorial is under /langgraph/tutorials; previous /coagents path was outdated. */}
</Callout>
...
<Callout>
  Unsure which approach to use? We recommend starting with the [Interrupt](/langgraph/human-in-the-loop/interrupt-flow) flow. {/* docs-fix: update to LangGraph HITL route; previous /coagents path was inconsistent. */}
</Callout>
  ...
  cards={[
    {
      ...
      href: "/langgraph/human-in-the-loop/interrupt-flow" // docs-fix: correct LangGraph route (was /coagents/human-in-the-loop/...).
    },
    {
      iconKey: "share2",
      title: "Node-based",
      description: "Utilize nodes and tools to create LLM-driven Human-in-the-Loop workflows.",
      href: "/langgraph/human-in-the-loop/node-flow" // docs-fix: correct LangGraph route (was /coagents/human-in-the-loop/...).
    }
  ]}
/>
