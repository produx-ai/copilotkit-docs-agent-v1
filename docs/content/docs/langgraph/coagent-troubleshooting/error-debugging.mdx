...
<CopilotKit
  ...
  onError={(errorEvent) => {
    // Production error observability
    if (errorEvent.type === "error") {
      // Replace the following with your own logger/monitoring implementation.
      // Reason: these are placeholders; commenting them avoids copy-paste errors
      // with undeclared variables like `logger` / `monitoring`.
