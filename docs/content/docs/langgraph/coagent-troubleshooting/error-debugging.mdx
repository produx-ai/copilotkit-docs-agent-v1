...
export default function App() {
  // docs-fix: define a minimal `analytics` placeholder so this snippet is copy-paste safe.
  const analytics = { track: (..._args: any[]) => {} };
...
```tsx
// docs-fix: `CopilotErrorEvent` is not exported from `@copilotkit/react-core`; import it from `@copilotkit/shared`.
