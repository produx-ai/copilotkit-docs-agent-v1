...
<Steps>
  <Step>
    ### Prerequisites
    Install any required dependencies for the examples below (for TypeScript projects using schemas):
    ```bash
    npm install zod
    ```
  </Step>
...
<Callout type="info" title="Important">
  In order to render a tool call in the UI, the name in `useRenderToolCall` must match the name of the tool.
</Callout>
