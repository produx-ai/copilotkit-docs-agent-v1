...
All LangGraph agents are stateful. This means that as your agent progresses through nodes, a state object is passed between them preserving
the overall state of a session. CopilotKit allows you to render this state in your application with custom UI components, which we call **Agentic Generative UI**. {/* docs-fix: Correct typo (“perserving” -> “preserving”) for clarity and professionalism. */}
    ...
    <Tabs groupId="language_langgraph_agent" items={['Python', 'TypeScript']} default="Python" persist>
      <Tab value="Python">
        ...
        async def chat_node(state: AgentState, config: RunnableConfig):
            ...
            return {"messages": response, "searches": state["searches"]}  # docs-fix: Ensure the node returns the final state update; emitted intermediate state does not persist unless returned when the node completes.
      </Tab>
      <Tab value="TypeScript">
        ```typescript title="agent-js/src/agent.ts"
        ...
        async function chat_node(state: AgentState, config: RunnableConfig) {
          ...
          // We can call copilotkit_emit_state to emit updated state
          // before a node finishes
          await copilotkitEmitState(config, { ...state, searches });
          ...
          return { messages: response, searches }; // docs-fix: Avoid relying on in-place mutation; persist state by returning the final node update.
        }
        ```
      </Tab>
    </Tabs>
