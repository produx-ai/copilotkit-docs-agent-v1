...
This can be utilized for to let [your agent control the UI](/langgraph/frontend-actions), [generative UI](/langgraph/generative-ui)<!-- doc-fix: avoid self-referencing link; point to Generative UI overview page per verification report -->, or for Human-in-the-loop interactions.
