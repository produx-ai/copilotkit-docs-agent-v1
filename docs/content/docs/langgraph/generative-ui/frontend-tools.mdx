...
This can be utilized to let [your agent control the UI](/langgraph/frontend-actions), [generative UI](/langgraph/generative-ui/frontend-tools), or for Human-in-the-loop interactions. {/* docs-fix: Grammar cleanup (“for to let” -> “to let”) while preserving the existing links and meaning. */}
