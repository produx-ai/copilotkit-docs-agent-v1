...
<TailoredContent
    ...
    >
        ...
        To
        ```typescript
        import { CopilotRuntime } from "@copilotkit/runtime"; // docs-fix: LangGraphAgent is no longer imported from "@copilotkit/runtime" (deprecated)
        import { LangGraphAgent } from "@copilotkit/runtime/langgraph"; // docs-fix: use the dedicated langgraph entrypoint per runtime deprecation notice
...
<Steps>
    ...
    <Step>
        ...
        ```typescript
        import { CopilotRuntime } from "@copilotkit/runtime"; // docs-fix: make the snippet self-contained (CopilotRuntime is referenced below)
        import { LangGraphHttpAgent } from "@copilotkit/runtime/langgraph"; // docs-fix: import from langgraph entrypoint (root export is deprecated)
        ...
        <Accordions>
            <Accordion title="What about the old LangGraph config parameter which was used to configure the agent?">
                ...
                Configure the agent via the [`useCoAgent`](/reference/hooks/useCoAgent) hook or with the new `config` parameter on the agent class. {/* docs-fix: link to the existing `useCoAgent` reference page for specific guidance */}
            </Accordion>
        </Accordions>
                The `langgraph_config` option doesn’t exist on the new agent.
