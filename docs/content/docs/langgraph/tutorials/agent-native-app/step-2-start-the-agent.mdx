...
This opens LangGraph Studio in your browser where you can visualize and interact with your agent. <!-- docs-fix: typo "brwoser" -> "browser" -->

<Callout>
The agent will be available at http://localhost:8000 for later steps. <!-- docs-fix: local dev typically uses http (command starts without TLS); avoid implying https by default -->
</Callout>
...
You can submit a message to the agent by adding a message to the `messages` state variable and then clicking "Submit". This agent is pared
down for this tutorial and is meant to represent a simple research agent. <!-- docs-fix: idiom is "pared down" (was "paired down") -->
...
## Recap <!-- docs-fix: remove duplicated header introduced during edits -->
Now we have a running agent! It will be available at `http://localhost:8000`. <!-- docs-fix: keep recap consistent with earlier http URL --> Here's what we did:
