...
from copilotkit.langgraph import copilotkit_customize_config, copilotkit_emit_state # [!code ++] <!-- docs-fix: use LangGraph SDK import; copilotkit.langchain is deprecated -->

# ...
@tool("section_writer", args_schema=SectionWriterInput, return_direct=True)
async def section_writer(research_query, section_title, idx, state, config):  # <!-- docs-fix: add config param; body customizes config -->
