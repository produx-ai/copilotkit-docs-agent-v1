...
- For the `initialState` parameter, we pass the initial state of the LangGraph agent which is already defined in `@/lib/types.ts`. <!-- docs-fix: avoid travel-tutorial copy/paste reference to `@/lib/trips.ts` -->
</Steps>
