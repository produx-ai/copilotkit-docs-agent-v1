...
we'll be adding `useCopilotAction` hooks for each tool call with the `renderAndWaitForResponse` option.
...
export const TripsProvider = ({ children }: { children: ReactNode }) => {
  ...
  useCopilotAction({ 
    ...
    renderAndWaitForResponse: AddTrips,
  });
  ...
  useCopilotAction({
    ...
    renderAndWaitForResponse: EditTrips,
  });
  ...
  useCopilotAction({
    ...
    renderAndWaitForResponse: (props) => DeleteTrips({ ...props, trips: state.trips }),
  });
```
...
export const DeleteTrips = ({ args, status, respond, trips }: DeleteTripsProps) => {
  const tripsToDelete = trips.filter(trip => args.trip_ids.includes(trip.id));
...
export const ActionButtons = ({ status, respond, approve, reject }: ActionButtonsProps) => (
  <div className="flex gap-4 justify-between">
    <Button 
      className="w-full"
      variant="outline"
      disabled={status === "complete" || status === "inProgress"} 
      onClick={() => respond?.("CANCEL")} // [!code highlight]
    >
      {reject}
    </Button>
    <Button 
      className="w-full"
      disabled={status === "complete" || status === "inProgress"} 
      onClick={() => respond?.("SEND")} // [!code highlight]
    >
      {approve}
    </Button>
  </div>
...
then the `respond?.("SEND")` is called. If the user clicks the `Cancel` button then the `respond?.("CANCEL")` is called. This is how the 
