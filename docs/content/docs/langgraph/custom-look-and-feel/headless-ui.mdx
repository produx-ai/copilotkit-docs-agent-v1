...
When using the `useCopilotChatHeadless_c` hook, you can render the returned `interrupt` UI element to interrupt the chat.
<!-- Patch: clarify that `interrupt` is a renderable React node/component, not a callable function, matching the example usage `{interrupt}`. -->
