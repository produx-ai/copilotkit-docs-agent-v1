...
<!-- Doc fix: `interrupt` returned by `useCopilotChatHeadless_c` is a renderable value (string | ReactElement | null), not a callable function. -->
When using the `useCopilotChatHeadless_c` hook, you can render the `interrupt` property to display the interrupt UI.
