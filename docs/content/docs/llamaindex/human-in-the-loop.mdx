...
Human-in-the-Loop (HITL) tools build on frontend tools by enabling your LlamaIndex agent to invoke client-side functions that require human validation or input before proceeding. Execution occurs in the user's browser, allowing direct access to frontend state for gated interactions.

This is particularly useful for workflows where [your agent needs user approval for actions](/llamaindex/frontend-actions), generates [interactive UI choices](/llamaindex/generative-ui/frontend-tools), or pauses for human decisions in the loop.

In this guide, we specifically cover HITL implementations where agent responses depend on user selections or confirmations.

## When should I use this?

Use HITL tools when you need your agent to:
- Prompt users for selections or approvals before continuing (e.g., choosing between options).
- Access user-specific frontend context like sessionStorage or component state for decision-making.
- Ensure agent actions are gated by human input, preventing automated errors in critical flows.
- Integrate with UI for interactive confirmations or choices beyond simple alerts.
