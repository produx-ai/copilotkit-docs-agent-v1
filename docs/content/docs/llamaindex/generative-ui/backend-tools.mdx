...
def get_weather(location: str) -> str:  # NOTE: use snake_case so the exposed tool name matches the frontend renderer.
...
<Callout type="info" title="Important">
  In order to render a tool call in the UI, the `name` in `useRenderToolCall` must match
  the tool name exactly (including casing).
</Callout>
...
const YourMainContent = () => {
  ...
  useRenderToolCall({
    name: "get_weather",
    render: ({ status, args, result }) => { // NOTE: `result` is also available for complete tool calls.
      return (
