...
<Steps>
  ...
  <Step>
    ...
    from llama_index.protocols.ag_ui.events import StateSnapshotWorkflowEvent  # NOTE: This import path is also used by the LlamaIndex starter example in this repo.
    async def addSearch(
        ctx: Context,
        query: Annotated[str, "The search query to add."]
    ) -> str:
        """Add a search to the agent's list of searches."""
        async with ctx.store.edit_state() as global_state:  # NOTE: This pattern is used in other LlamaIndex shared-state docs in this repo.
  ...
  <Step>
    ...
    function YourMainContent() {
      ...
      // [!code highlight:3]
      // [!code highlight:3]
      const { state } = useCoAgent<AgentState>({ // NOTE: `useCoAgent` returns an object with a top-level `state` property.
        name: "my_agent", // MUST match the agent name in CopilotRuntime
      })
      // ...
