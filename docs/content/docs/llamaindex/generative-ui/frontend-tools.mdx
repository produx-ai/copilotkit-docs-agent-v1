...
This can be utilized to let [your agent control the UI](/llamaindex/frontend-actions), [generative UI](/llamaindex/generative-ui), or for Human-in-the-loop interactions. <!-- Patch: avoid circular self-link; point to the Generative UI overview page instead of the current page. -->
    ...
    <Step>
        ...
        export function Page() {
          ...
          useFrontendTool({
            ...
            handler(args) { <!-- Patch: match the current `useFrontendTool` handler signature (single `args` object). -->
              const { name } = args;
