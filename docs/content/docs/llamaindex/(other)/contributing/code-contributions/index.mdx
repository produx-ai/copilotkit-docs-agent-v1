...
The easiest way to get a hot-reload development environment is to use the ready-made development script with one of the CoAgent examples. This allows you to work on CopilotKit internals while seeing your changes applied in real-time.

As a prerequisite, ensure you have GNU parallel and langgraph CLI installed.

From the root `CopilotKit/` directory, run the following command in a separate terminal:

```bash
./scripts/develop/example.sh coagents-starter
```

This will start a development environment with hot reload. You can also run the example with the LangGraph platform backend:

```bash
./scripts/develop/example.sh coagents-starter langgraph-platform
```

Any changes you make to the CopilotKit packages will immediately be reflected in the running example.
```
