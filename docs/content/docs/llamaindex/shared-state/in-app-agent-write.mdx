...
<Steps>
  ...
  <Step>
    ...
    <Callout type="info">
      ...
      {/* NOTE: `useCoAgent` calls `agent.setState(...)` locally in the React hook; backend persistence depends on when/if the agent run is executed. */}
    </Callout>
...
// NOTE: These are exported by @copilotkit/runtime-client-gql (see CopilotKit/packages/runtime-client-gql/src/index.ts).
