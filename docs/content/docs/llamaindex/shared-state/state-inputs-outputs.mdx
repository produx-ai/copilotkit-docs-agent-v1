...
<Steps>
  <Step>
    ...
    initial_state = {
        "question": "",         # Input from user (example state field; you may still pass the actual user question via the message stream)
  ...
  <Step>
    ...
    async def answerQuestion(
        ...
        async with ctx.store.edit_state() as global_state:
            ...
            # NOTE: This mirrors the llamaindex starter example in this repo (see examples/llamaindex/starter/agent-py/agent/agent.py).
  ...
  <Step>
    ...
    function YourMainContent() { // [!code highlight]
      ...
      const askQuestion = async (newQuestion: string) => {
        ...
        // NOTE: `run`'s hint callback returns a CopilotKit `Message` (see `HintFunction` in @copilotkit/react-core), so we construct a user-message shaped object.
        try {
