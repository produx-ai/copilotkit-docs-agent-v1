...
<Tabs groupId="gen-ui-type" items={['Render a component', 'Fetch data & render', 'renderAndWaitForResponse (HITL)', 'Render strings', 'Catch all renders']}>
  ...
  <Tab value="Catch all renders">
    ...
    ```tsx
    import { useDefaultTool, CatchAllActionRenderProps } from "@copilotkit/react-core";
    {/* CORRECTED: Changed import from CatchAllToolRenderProps to CatchAllActionRenderProps to match actual type name in source code */}
    useDefaultTool({
