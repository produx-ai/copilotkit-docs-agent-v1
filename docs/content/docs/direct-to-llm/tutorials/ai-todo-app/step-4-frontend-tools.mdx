...
```tsx filename="lib/hooks/use-tasks.tsx" showLineNumbers {3-3,8-28,30-46,48-73}
...
- `parameters` is an array of parameter definitions that specify what inputs the tool accepts. Each parameter has a `name`, `type`, `description`, and `required` field.
- `handler` is a function that will be called when the tool is triggered. The parameters are automatically type-checked!
