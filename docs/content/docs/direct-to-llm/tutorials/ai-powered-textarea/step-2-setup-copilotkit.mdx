...
Now that we have our email app running, we're ready to integrate CopilotKit. For this tutorial, we will install the following dependencies:
...
```tsx title="app/page.tsx" showLineNumbers {5,10}
"use client";
...
- Styles are automatically injected by `@copilotkit/react-textarea` in current versions, so a manual `styles.css` import is optional (include it only if supporting older versions).
</TailoredContent>
