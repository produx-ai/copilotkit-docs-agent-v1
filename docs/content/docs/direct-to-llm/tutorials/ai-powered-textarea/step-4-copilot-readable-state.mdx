...
- For the `value` property, we pass the entire state; the hook serializes it for you.
...
## Try it out!
Your `<CopilotTextarea />` now uses this readable state for improved autocompletions.
