...
```tsx title="lib/hooks/use-emails.tsx"
...
- For the `value` property, we pass the entire state. The hook automatically serializes it to a string using JSON.stringify.
